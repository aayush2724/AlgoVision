...
**Status: Phases 0–5 DONE. Catalog batches 1–15 DONE — 61 algorithms live,
1255 backend tests passing. Tier A (everything buildable on existing views) is
COMPLETE; only Tier B (needs new renderers / directed-graph editor) remains.**
...
tested. What remains is all Tier B — each needs a new renderer or interaction
model, so none is a plain batch:
- A*, Bellman–Ford, Floyd–Warshall — need a directed / negative-weight / goal
  graph **editor** (see the graph-model note above).
